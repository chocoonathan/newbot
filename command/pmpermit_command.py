...
async def pmpermit_cmd(client, message):
    ...
    elif variable.lower() == "inline":
        if value.lower() in ["disable", "off"]:
            await dB.set_var(client.me.id, "DISABLE_PMINLINE", True)
            return await proses.edit(
                f"{em.sukses}**PMPermit inline has been disable.**"
            )
        await dB.remove_var(client.me.id, "DISABLE_PMINLINE")
        return await proses.edit(f"{em.sukses}**PMPermit inline has been enable.**")
...
async def PMPERMIT(client, message):
    ...
    text_send = pmtok if pmtok else DEFAULT_TEXT
    ...
    if not await dB.get_var(gw, "DISABLE_PMINLINE"):
        try:
            uniq = f"{str(uuid4())}"
        ...
        parse_text = await Tools.escape_filter(message, text_send, Tools.parse_words)
        return await message.reply(parse_text)
