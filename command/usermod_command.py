...
async def story_cmd(client, message):
    ...
    if len(message.command) < 2:
        return await proses.edit(
            f"{em.gagal}<b>Please give query [post, cek, get, del, or copy]</b>"
        )
    ...
    if query == "cek":
        ...
        await client.delete_stories(chat_id="me", story_ids=int(story_id))
        return await proses.edit(
        ...
        entities = reply.caption_entities or ""
        try:
        ...
        try:
            user, story_id = Tools.extract_story_link(link)
            story = await client.get_stories(user, story_id)
            if getattr(story, "video"):
                file_id = story.video.file_id
            else:
                file_id = story.photo.file_id
            True if getattr(story, "video") else False
            posted = await client.send_story(
                chat_id="me",
                media=file_id,
            ...
            this = await client.export_story_link("me", posted.id)
            return await proses.edit(
                f"{em.sukses}<b>Succesfully upload to [Story]({this})!</b>",
                disable_web_page_preview=True,
            )
