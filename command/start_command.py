...
async def start_home(client, message):
    ...
    if len(message.command) > 1 and message.command[1].startswith("yt_"):
        try:
            params = message.command[1].split("_")
            query_type = params[1]
            encoded_video_id = params[2]
            video_id = encoded_video_id.replace("-UNDERSCORE-", "_")
            return await process_yt_download(client, message, video_id, query_type)
        except Exception as e:
            logger.error(f"Start download error: {e}")
            return await message.reply("❌ Error processing download request")
...
async def process_yt_download(client, message, video_id, query_type):
    try:
        is_video = query_type == "video"
        processing_msg = await message.reply(
            f"📥 **Sedang mendownload {'video' if is_video else 'audio'}...**\n\n"
        ...
        (
            ...
            thumb,
            data_ytp,
        ) = await youtube.download(video_id, as_video=is_video)
        ...
        now = time.time()
        await processing_msg.edit("🔄 **Sedang mengupload...**")
        if is_video:
