...
async def inline_help_func(_):
    ...
    buttons_owner = ikb([[("👤 My Owner", OWNER_ID, "user_id")]])
    answerss = [
        InlineQueryResultArticle(
...
<b>╭─   👤 User: {full}</b>
<b>├ 💎 Plan: {plan_teks}</b>
<b>├ 🔰 Prefix:</b> <code>{' '.join(prefix)}</code>
<b>├ 📦 Plugins:</b> <code>{len(visible_helpable)}</code>
<b>╰ 🗂️ Categories:</b> <code>{total_categories}</code>
...
🔗 <a href="{audio['url_suffix']}">Youtube Link</a>\n"""
        buttons.append(
            [
                (
                    "⬇️ Download " + audio["title"][:20],
            ...
            if data.get("status") and data.get("data"):
                ...
                if results_data.get("anime"):
                    for anime in results_data["anime"][:3]:
                        await add_anime_result(answers, anime, "🎬 Anime")

                if results_data.get("karakter"):
                    for char in results_data["karakter"][:3]:
                        await add_character_result(answers, char, "👤 Karakter")
                if results_data.get("artikel"):
                    for article in results_data["artikel"][:2]:
                        await add_article_result(answers, article, "📰 Artikel")
                if not answers:
...
_📖 Klik untuk melihat detail anime di OtakOtaku_</blockquote>"""

    answers.append(
        InlineQueryResultArticle(
            title=f"🎬 {title[:50]}...",
            description=f"Anime • Detail series",
            input_message_content=InputTextMessageContent(
                msg, disable_web_page_preview=False
            ),
            thumb_url=image_url if image_url else "https://otakotaku.com/favicon.ico",
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("🌐 Lihat di OtakOtaku", url=url)]]
            ),
        )
    )
...
_📖 Klik untuk melihat detail karakter di OtakOtaku_</blockquote>"""

    answers.append(
        InlineQueryResultArticle(
            title=f"👤 {title[:50]}...",
            description=f"Karakter • Detail tokoh",
            input_message_content=InputTextMessageContent(
                msg, disable_web_page_preview=False
            ),
            thumb_url=image_url if image_url else "https://otakotaku.com/favicon.ico",
...
_📖 Klik untuk membaca artikel lengkap di OtakOtaku_</blockquote>"""

    answers.append(
        InlineQueryResultArticle(
            title=f"📰 {title[:50]}...",
            description=f"Artikel • Berita anime",
            input_message_content=InputTextMessageContent(
                msg, disable_web_page_preview=False
            ),
            thumb_url=image_url if image_url else "https://otakotaku.com/favicon.ico",
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("🌐 Baca Artikel", url=url)]]
            ),
        )
    )
...
_Silahkan buka link di atas untuk pencarian manual di OtakOtaku_"""
    ...
    answers.append(
        InlineQueryResultArticle(
            ...
            thumb_url="https://otakotaku.com/favicon.ico",
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("🌐 Cari di OtakOtaku", url=search_url)]]
            ),
        )
    )
...
__🎯 Results {index} from YouTube__</blockquote>"""
    buttons = ikb(
        [
            [("🎥 Watch in YouTube", url, "url")],
            [
                ("🎬 Video", f"yt_video_{video_id}"),
                ("🎵 Audio", f"yt_audio_{video_id}"),
            ],
            [("🔍 Search Again", text, "switch_inline_query_current_chat")],
        ]
    )
    answers.append(
        InlineQueryResultArticle(
            title=f"🎬 {short_title}",
            description=f"{channel} • {duration} • {views_clean} views",
            input_message_content=InputTextMessageContent(
                msg, disable_web_page_preview=False
            ),
            thumb_url=thumbnail,
...
_Silahkan buka link di atas untuk pencarian manual di YouTube_"""
    answers.append(
        InlineQueryResultArticle(
            title=f"🔍 Cari di YouTube: {text[:45]}...",
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [InlineKeyboardButton("🔍 Open in YouTube", url=search_url)],
                    [
                        InlineKeyboardButton(
                            "🔍 Search Again", switch_inline_query_current_chat=text
                        )
                    ],
                ]
            ),
    ...
    try:
        ...
        if data_result.get("status_code") != 200:
            results.append(
                InlineQueryResultArticle(
                    ...
                    input_message_content=InputTextMessageContent(
                        message_text="❌ Failed to search LK21. Please try again later.",
                        parse_mode=enums.ParseMode.HTML,
                    ),
                )
            )
        ...
        for item in search_results[:50]:
            ...
            message_text = (
                ...
                f"<code>┣</code> <b>Type:</b> {emoji} {item_type}\n"
                f"<code>┣</code> <b>Year:</b> {year}\n"
                f"<code>┣</code> <b>Quality:</b> {kategori}\n"
                f"<code>┣</code> <b>Page:</b> <a href='{link}'>LK21</a>\n"
                f"<code>┗</code> <b>Download:</b> <a href='{dl_link}'>Click Here</a>\n\n"
            ...
            buttons = [
                [
                    InlineKeyboardButton("🎬 View Details", url=link),
            ...
            results.append(
                InlineQueryResultArticle(
                    ...
                    description=f"{item_type} • {year} • {kategori}",
                    input_message_content=InputTextMessageContent(
