...
from config import API_BOTCAHX, API_MAELYN, SUDO_OWNERS
from database import dB
from helpers import (HTML, ApiImage, Emoji, Message, Quotly, Sticker,
                     TelegramAPI, Tools, animate_proses)
...
async def iqc_cmd(client, message):
    em = Emoji(client)
    await em.get()
    prompt = client.get_text(message)
    ...
    if not prompt:
        return await message.reply(
            ...
            f"Example: `{message.text.split()[0]} bajingan kamu`"
        )
    proses = await animate_proses(message, em.proses)
    clean_prompt = HTML.clean_text(prompt)
    url = f"https://api.botcahx.eu.org/api/maker/iqc?text={clean_prompt}&apikey={API_BOTCAHX}"
    try:
        ...
        await proses.delete()
        return await message.reply_photo(img)
    except Exception as err:
        await proses.delete()
        return await message.reply(f"{em.gagal}**Error:** {str(err)}")
...
async def vremover_cmd(client, message):
    ...
    if not rep or not (rep.audio or rep.video or rep.voice):
        return await prs.edit(f"{em.gagal}**Please reply to audio or video!!**")

    telegram = TelegramAPI()
    audio_telegram = rep.audio or rep.voice
    video_telegram = rep.video
    max_limit = 104857600

    if audio_telegram:
        if audio_telegram.file_size > max_limit:
            return await prs.edit(
                f"{em.warn}<b>Audio size is too large! The maximum media size limit is 100Mb.</b>"
            )
        file_path = await telegram.get_filepath(audio=audio_telegram)
    elif video_telegram:
        if video_telegram.file_size > max_limit:
            return await prs.edit(
                f"{em.warn}<b>Video size is too large! The maximum media size limit is 100Mb.</b>"
            )
        file_path = await telegram.get_filepath(video=video_telegram)
    if await telegram.download(client, message, prs, file_path):
        if not file_path:
            return await prs.edit(
                f"{em.gagal}**Failed to download media. Please try again later.**"
            )

        try:
            arg = await Tools.maelyn_upload(file_path)
            url = f"https://api.botcahx.eu.org/api/tools/voiceremover?url={arg}&apikey={API_BOTCAHX}"
            respon = await Tools.fetch.get(url)
            await prs.delete()
            ex = await message.reply(f"{em.proses}**Try to extract of instruments...**")
            if respon.status_code != 200:
                return await ex.edit(f"{em.gagal}**Please try again later!!**")
            ...
            return await ex.edit(f"{em.gagal}**ERROR:** {str(er)}")
