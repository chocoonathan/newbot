...
@CMD.UBOT("kick|delkick", group=True, admins=True)
async def _(client, message):
    return await kick_cmd(client, message)


@CMD.UBOT("ban|delban|unban", group=True, admins=True)
async def _(client, message):
...
@CMD.UBOT("mute|delmute|unmute", group=True, admins=True)
async def _(client, message):
    return await mute_cmd(client, message)


@CMD.UBOT("zombies", group=True, admins=True)
async def _(client, message):
    return await zombies_cmd(client, message)


@CMD.UBOT("report", group=True)
async def _(client, message):
    return await report_cmd(client, message)


@CMD.UBOT("fullpromote|promote|demote", group=True, admins=True)
async def _(client, message):
    return await promote_cmd(client, message)


@CMD.UBOT("title|group", group=True, admins=True)
async def _(client, message):
