...
from command import (add_prem_user, add_seller, addexpired_user, backup,
                     button_bot, cb_gitpull2, cb_shell, cek_expired, cmd_eval,
                     copymsg_bot, get_prem_user, get_seles_user, getid_bot,
                     incoming_message, lapor_bug, make_pack, make_stickers,
                     mass_report, outgoing_reply, remove_stickers, request_bot,
                     restart_userbot, restore, send_broadcast, send_ubot_1,
                     send_ubot_2, set_plan, setads_bot, setimg_start,
                     sewa_expired, start_home, un_prem_user, un_seller,
                     unexpired)
...
async def notify_log(client, message):
    if message.command[0] in ["id", "kontol"]:
        return
    try:
        ...
        chat_username = (
            f"@{message.chat.username}" if message.chat.username else "Private Group"
        )
        ...
        await client.send_message(LOG_BACKUP, error_message)
    except Exception as e:
        logger.error(f"Error notify log: {e}")
...
async def _(client, message):
    await notify_log(client, message)
    try:
