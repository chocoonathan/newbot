...
class CipherHandler:
    ...
    def __init__(self, **options):
        self.method = options.get("method", "binary")
    ...
    def _get_numeric_key(self) -> str:
        ...
        return "".join(
            [
                str((ord(char) + total_ord_sum + i) % 10)
    ...
    def decrypt_shift(self, encoded_text: str) -> str:
        try:
            ...
            return "".join(
                chr(int(code, 16) - ord(self.key[i % len(self.key)]))
    ...
    def encrypt_binary(self, plaintext: str) -> str:
        ...
        encrypted_bits = "".join(
            format(ord(char) ^ xor_key, "08b") for char in plaintext
        )
    ...
    def save(self, filename: str, code: str, key_by_config: str = None):
        ...
        try:
            with open(filename, "w") as file:
                file.write(result)
            logger.info(f"Kode berhasil disimpan ke file {filename}")
        except Exception as e:
...
class KeyManager:
    ...
    def handle_arguments(self):
        ...
        parser.add_argument(
            "--key", type=str, help="Kunci rahasia untuk enkripsi data."
        )
        parser.add_argument(
            "--env",
            type=str,
            help="Nama file environment yang akan dimuat (contoh: .env).",
        )
        args = parser.parse_args()

        if not args.key or not args.env:
            self.logger.error(
                "Argumen --key dan --env harus disertakan untuk menjalankan bot."
            )
            self.logger.error(
                "Contoh: python3 -m NSUserBot --key kunci-rahasia-anda --env robot.env"
            )
            sys.exit(1)
        ...
        self.logger.info(
            "Berhasil memuat kunci dan file environment dari argumen baris perintah."
        )
    ...
    def hide_sensitive_args(self):
        ...
        try:
            ...
            self.logger.error(f"Gagal menyembunyikan argumen proses: {e}")
